...
def _format_classification_results(models_df, y_test):
    ...
    score_col = next(
        (col for col in metric_priority if col in models_df.columns),
        None
    )
...
def calculate_feature_importance(
    ...
    """
    Production-grade feature importance extractor.

    Supports:
    - Tree models via feature_importances_
    - Linear models via coef_
    - Any sklearn-compatible model via permutation importance fallback
    ...
    if not hasattr(X_train, "columns"):
        raise ValueError(
            "X_train must be a pandas DataFrame with column names."
        )
    ...
    if hasattr(model, "feature_importances_"):
        ...
        importances = sorted(
            [
                ...
                for f, i in zip(feature_names, values)
            ],
    ...
    # ---------------------------------------------
    # 2. LINEAR MODEL COEFFICIENT IMPORTANCE
    # ---------------------------------------------
    elif hasattr(model, "coef_"):

        coef = model.coef_

        # Multi-class classifiers sometimes return 2D arrays
        ...
        importances = sorted(
            [
                ...
                for f, i in zip(feature_names, coef)
            ],
            key=lambda r: r["importance"],
            reverse=True,
        )[:top_n]
